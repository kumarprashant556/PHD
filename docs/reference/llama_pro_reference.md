...
> **MetaMath SFT note (Appendix C)**: LLaMA-Pro further fine-tuned on MetaMath (Yu et al.,
> 2023) as part of the SFT stage, yielding **GSM8K = 78.4** and **MATH = 30.3**, surpassing
> Mistral-7B (77.7 / 28.2). MetaMath is therefore one of the driving datasets behind
> LLaMA-Pro-Instruct's math scores. See Paper B journal entry 2026-06-17 for INCA's
> MetaMath adoption plan.
...
### 6.6 Figure 5 — Training loss vs. block count (scalability)

Loss consistently decreases regardless of how many blocks are added. More blocks → faster loss
drop. BUT lower training loss does not guarantee better downstream task performance (key caveat).
This is why they always evaluate on Unfair-ToS, not just training loss.

**INCA implication**: our saturation detector uses loss-plateau as ONE of four signals — not
the primary one — precisely because low loss ≠ saturated learning. RIR + CKA + grad-norm
confirm that the loss has stopped producing useful representations, not just that it is low.

### 6.7 Figure 6 — Token distribution shift after block expansion

Token distribution between LLaMA-2-7B and LLaMA-Pro measured on Alpaca general questions:
- **Unshifted**: 92.6% of tokens unchanged
- **Marginally shifted**: 6.8%
- **Significantly shifted**: 0.7%

Conclusion: block expansion (with frozen original layers) causes almost no shift in general
language generation behaviour. This is their empirical proof of "general ability preservation."

**INCA equivalent**: CKA drift on frozen blocks across periods. If CKA of frozen block
representations stays ≥ 0.95 before and after a grow event, we have the same preservation
guarantee. This measurement goes in Paper B Figure 3.

### 6.8 Figure 7 — Same SFT data: LLaMA-Pro > LLaMA2-7B on all tasks

Fine-tuning LLaMA2-7B and LLaMA-Pro on the **identical instruction dataset**:

| Task | LLaMA2-7B | LLaMA-Pro | Δ |
|---|---|---|---|
| GSM8K | 63.61 | 64.67 | +1.06 |
| MATH | 19.6 | 21.2 | +1.6 |
| HumanEval | 39.0 | **47.56** | **+8.56** |
| MBPP | 31.2 | **41.4** | **+10.2** |

Conclusion: the block expansion pre-training stage encoded real domain knowledge — it is not
just a parameter count increase. LLaMA-Pro consistently outperforms LLaMA2-7B even when both
get identical SFT data. The gap is largest on code (HumanEval +8.56, MBPP +10.2).

**INCA direct analog (most important figure to replicate)**:
Train B6 (fixed schedule, same block count) and INCA (saturation-driven) on identical domain
data, same seeds. If INCA > B6 on final F1, this proves that saturation-driven timing encodes
more domain knowledge than fixed-schedule expansion — the same argument LLaMA-Pro makes for
pre-training vs. pure SFT. This becomes INCA Paper B Figure 2 (or Table 1 + narrative).

### 6.9 §4.5 Key qualitative findings (not in tables)

1. **LoRA (rank=1024) preserves general ability but fails at domain distribution shift.**
   Quote: *"LoRA effectively preserves the general ability, it struggles to model the
   distribution of a new domain."* Training loss (Figure 5): LoRA loss plateau is higher
   than block-expansion methods.
   → Maps to our B4 (ER) + B3 (EWC) baselines — both are parameter-efficient methods that
   should show the same weakness on domain F1.

2. **Full fine-tuning causes significant general ability drop.**
   Quote: *"full fine-tuning results in a more significant drop in general performance."*
   → Maps to our B1 (FT-all) baseline — should show large BWT (forgetting P1 after P2/P3).

3. **Interleaved > Suffix > Prefix stacking.**
   Bottom stacking disrupts model foundation, causing error propagation. Top stacking
   preserves initial performance but weaker domain specialisation.
   → INCA uses interleaved growth (layer_manager.py). No need to ablate this, but cite
   LLaMA-Pro §4.5 as justification for our placement choice.

4. **8 blocks is the sweet spot; 16 blocks ≈ 8 blocks.**
   Diminishing returns beyond 8 blocks; MoE ≈ 4-block performance but with more parameters.
   → INCA's `n_max_blocks=8` config directly motivated by this finding. But our point is
   that INCA finds the *right* count adaptively (0 to 8), whereas LLaMA-Pro fixes it at 8.

### 6.10 Mistral-Pro (Table 4)
