...
def tokenize_batch(
    ...
    - Decoder : target_text → labels  (pad → -100)
    """
    ...
    # Replace pad id with -100 so loss ignores padding positions
    pad_id = tokenizer.pad_token_id
    model_inputs["labels"] = [
        [(tok if tok != pad_id else -100) for tok in seq]
        for seq in label_enc["input_ids"]
    ]
    return model_inputs
        ...
        while True:
            ...
            for k in list(batch.keys()):
                try:
                    ...
                    pass   # leave as list if not stackable
