...
from training.cl_runner import (
    Period, make_loader, model_dtype, Seq2SeqDataset, TextDataset,
...
class B2ReplayOnly:
    ...
    def train_period(self, period, scheduler=None, loss_logger=None, text_logger=None):
        ...
        ds = Seq2SeqDataset(period.train_items, self._tokenizer, max_seq, max_ans)
        from torch.utils.data import DataLoader
        # drop_last=False: small periods (e.g. 2019 with 1 doc) must not be
        # silently dropped.  The last partial batch is fine for loss averaging.
        dl = DataLoader(ds, batch_size=cur_bs, shuffle=True, drop_last=False)
        ...
        for epoch in epoch_bar:
            ...
            for ms, batch in enumerate(batch_bar, 1):
                ...
                if self._buffer and rep_bs > 0:
                    ...
                    rep_ds  = Seq2SeqDataset(samples, self._tokenizer, max_seq, max_ans)
                    if len(rep_ds):
                        items = [rep_ds[i] for i in range(len(rep_ds))]
                        rb    = {k: torch.stack([x[k] for x in items]) for k in items[0]}
                        rl    = self._forward(rb)
