...
> **Dataset**: 5-domain sequential curriculum — Math → Code → Science → Medical → Commonsense  
> **Model**: FLAN-T5-large (780 M parameters)  
> **Hardware target**: single Apple M-series Mac (MPS) or 1× GPU (CUDA, ≥16 GB)
...
|---|---|---|
| Python | 3.10 | 3.11 |
| RAM | 16 GB | 32 GB unified (M4) / 24 GB VRAM (CUDA) |
| Disk (models + data cache) | 20 GB | 30 GB |
| Internet | Required on first run | HuggingFace Hub |
| GPU / MPS | Optional (MPS auto-detected) | Apple M4 MPS · RTX 4090 · A100 |

HuggingFace will auto-download and cache:
- `google/flan-t5-large` — model weights (~3 GB)
- `qwedsacf/competition_math` — math dataset (~100 MB)
- `flytech/python-codes-25k` — code dataset (~200 MB)
- `allenai/sciq` — science dataset (~20 MB)
- `medmcqa` — medical MCQ dataset (~150 MB)
- `tau/commonsense_qa` — commonsense dataset (~15 MB)
...
|---|---|---|---|---|
| `P1_math` | Competition Math | `qwedsacf/competition_math` | ~100 MB | 12,500 problem+solution pairs; subsampled to `n_per_period` |
| `P2_code` | Python codes 25k | `flytech/python-codes-25k` | ~200 MB | 49,000 instruction→program pairs; subsampled to `n_per_period` |
| `P3_science` | SciQ | `allenai/sciq` | ~20 MB | 13,679 science QA; train+val+test merged; subsampled to `n_per_period` |
| `P4_medical` | MedMCQA | `medmcqa` | ~150 MB | 182,000 medical MCQ; train split; subsampled to `n_per_period` |
| `P5_commonsense` | CommonsenseQA | `tau/commonsense_qa` | ~15 MB | 10,962 commonsense QA; train+val merged; subsampled to `n_per_period` |

### 3.2 Model weights

| Component | HF path | Size |
|---|---|---|
| FLAN-T5-large (encoder-decoder, 780 M params) | `google/flan-t5-large` | ~3 GB |

### 3.3 Natural Q/A framing

Every domain uses its **natural question-answer structure** — no arbitrary completion split:

| Period | Input prefix | Input content | Target |
|---|---|---|---|
| P1_math | `solve: ` | problem statement | full solution (multi-step) |
| P2_code | `code: ` | natural-language instruction | complete Python program |
| P3_science | `answer: ` | support passage + question | correct answer string |
| P4_medical | `answer: ` | question + A/B/C/D options | correct option text [+ explanation] |
| P5_commonsense | `answer: ` | question + A/B/C/D/E options | correct option text |

This is handled automatically by `data/domain_sequential.py` — no preprocessing script needed.
The Q/A framing eliminates target-token leakage by construction: no target token appears in the encoder input.
...
load_dataset('qwedsacf/competition_math', split='train')
load_dataset('flytech/python-codes-25k', split='train')
load_dataset('allenai/sciq', split='train')
load_dataset('allenai/sciq', split='validation')
load_dataset('allenai/sciq', split='test')
load_dataset('medmcqa', split='train')
load_dataset('tau/commonsense_qa', split='train')
...
```yaml
model_name:         "google/flan-t5-large"  # 780 M params (24 enc + 24 dec layers)
dataset:            "domain_sequential"      # math → code → science → medical → commonsense
n_per_period:       8000                     # items per domain
max_periods:        5                        # 5 domains
epochs_per_period:  3
batch_size:         4                        # effective batch = 32 (grad_accum=8 × 4)
lr:                 3.0e-4
