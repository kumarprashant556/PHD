...
def build_training_args(
    ...
    # MPS does not support pinned memory; setting pin_memory=True (the HF
    # Trainer default) wastes ~300 MB of extra RAM and logs a UserWarning.
    # Disable it unconditionally on MPS.
    if device == "mps":
        kw["dataloader_pin_memory"] = False

    prec = getattr(cfg, "precision", "bf16")
    # On MPS, Accelerate's AMP (bf16 or fp16) allocates BOTH a half-precision
    # working copy and an fp32 master copy — that's ~1.5× extra model memory
    # before the first training step.  For FLAN-T5-large (780 M params) this
    # pushes past the 27.2 GB MPS limit.  Use plain fp32 training on MPS:
    # activations stay float32 anyway (MPS silently falls back from bf16),
    # so we lose nothing in practice and save ~3.1 GB of allocator headroom.
    if device == "mps" and prec in ("bf16", "fp16"):
        logging.getLogger("capsel.runner").warning(
            "MPS device: disabling Accelerate AMP (%s→fp32). "
            "HF Trainer's AMP on MPS allocates fp32 master copy + half working "
            "copy — too much memory for FLAN-T5-large. "
            "Use a CUDA device for bf16/fp16 training.",
            prec,
        )
        prec = "fp32"
    # Safety: accelerate refuses bf16 on MPS unless torch ≥ 2.6.0.
    elif prec == "bf16" and device == "mps" and not _mps_bf16_supported():
        logging.getLogger("capsel.runner").warning(
