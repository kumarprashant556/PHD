...
class INCAConfig(BaseConfig):
    ...
    # ── Growth primitive (E-PRIM ablation) ───────────────────────────
    #   "expert"  G-EXPERT: deepcopy frozen block → warm-started trainable block (Paper B default)
    #   "vert"    G-VERT:   deepcopy + zero output projections → identity-init block (function-preserving)
    #   "horiz"   G-HORIZ:  Net2Net FFN width expansion — no new block; expands current block in-place
    growth_primitive: str = "expert"
    horiz_delta_w:    int = 512        # G-HORIZ: neurons added to FFN hidden dim per grow event
    ...
    # ── Eval / speed ──────────────────────────────────────────────────
    eval_batch_size:    int = 32    # batch size for _eval_accuracy (no grad, can be large)
    sat_eval_samples:   int = 200   # # samples for mid-training saturation checks
    sat_max_new_tokens: int = 64    # generation length for saturation F1 (not final BWT eval)
    max_new_tokens:     int = 256   # generation length for all eval (post-period + sat checks)

    # ── Saturation timeout ────────────────────────────────────────────
    patience_timeout:       int   = 50     # eval checks (loss-flat only) before timeout fires
    loss_plateau_min_delta: float = 5e-3   # loss range threshold for timeout gating

    # ── Metric early stop (primary convergence gate) ──────────────────
    early_stop_patience:  int   = 5        # consecutive non-improving checks before stalled
    early_stop_min_delta: float = 5e-3     # min F1 gain to count as improvement
